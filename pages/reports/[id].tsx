...
import { Report, AssessmentArea, DamageType, DamageSeverity } from "../../types/supabase";
...
const ReportDetailPage: React.FC = () => {
  ...
  const fetchReport = async (reportId: string) => {
    ...
    try {
      ...
      setIncidentDate(reportData.incident_date ? new Date(reportData.incident_date).toISOString().split('T')[0] : "");
  ...
  const handleAreaInputChange = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement>) => {
  ...
  const handleDeleteArea = async (areaId: string) => {
    if (!window.confirm("Are you sure you want to delete this assessment area?")) {
      return;
    }
  ...
  const renderAssessmentAreaForm = () => {
    return (
      <form onSubmit={handleAddArea} className="space-y-4">
        ...
        <div>
          <label htmlFor="damage_type" className="block text-sm font-medium text-gray-700 mb-1">
            Damage Type
          </label>
          <Select
            id="damage_type"
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              ...
              {property.square_footage && (
                <p>
                  <span className="font-medium">Size:</span> {property.square_footage} sq ft
                </p>
              )}
              {property.year_built && (
                <p>
        ...
        <div className="p-4">
          ...
          {isEditing ? (
            ...
            <div className="grid grid-cols-1 gap-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <h3 className="text-sm font-medium text-gray-500">Report Created</h3>
                ...
                {report.incident_date && (
                  <div>
                    ...
                    <p className="mt-1">{new Date(report.incident_date).toLocaleDateString()}</p>
                  </div>
                )}
                ...
                {report.submitted_at && (
                  <div>
                    ...
                    <p className="mt-1">{new Date(report.submitted_at).toLocaleDateString()}</p>
                  </div>
                )}
                ...
                {report.reviewed_at && (
                  <div>
                    ...
                    <p className="mt-1">{new Date(report.reviewed_at).toLocaleDateString()}</p>
                  </div>
                )}
              </div>
        ...
        <div className="p-4">
          ...
          {isAddingArea ? (
                ...
                <div key={area.id} className="border border-gray-200 rounded-lg p-4">
                  <div className="flex justify-between items-start">
                    <div>
                      ...
                      <p className="text-sm text-gray-600">
                        ...
                        <span className={`ml-1 px-1.5 py-0.5 rounded text-xs ${getSeverityColor(area.severity)}`}>
                      ...
                      {area.dimensions && (
                        <p className="text-sm text-gray-600 mt-1">
                          <span className="font-medium">Dimensions:</span> {area.dimensions}
                        </p>
                      )}
                  ...
                  <div className="mt-3">
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-2">
                      {report.images?.filter(img => img.assessment_area_id === area.id).map((image) => (
                        <div key={image.id} className="relative group">
                          ...
                          {image.ai_damage_type && (
                            <div className="absolute bottom-0 left-0 right-0 bg-black bg-opacity-60 text-white text-xs p-1 truncate">
                              ...
                              {image.ai_confidence && ` (${Math.round(image.ai_confidence * 100)}%)`}
                            </div>
                          )}
                        </div>
                      ))}
                      ...
                      {report.status === "draft" && (
                        <div className="h-24 w-24 border-2 border-dashed border-gray-300 rounded flex items-center justify-center">
                          <FileUpload 
                            bucket="reports" 
                            onUploadComplete={(urls) => handleImagesUpload(urls, area.id)}
            ...
            <div className="text-center py-6">
              <p className="text-gray-500">No assessment areas have been added yet.</p>
              {report.status === "draft" && !isAddingArea && (
                <button
                  onClick={() => setIsAddingArea(true)}
  ...
  const renderGeneralImages = () => {
    ...
    const generalImages = report.images?.filter(img => !img.assessment_area_id) || [];
        ...
        <div className="p-4">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
            {generalImages.map((image) => (
              <div key={image.id} className="relative group">
                <a href={image.storage_path} target="_blank" rel="noopener noreferrer">
                  <img 
                    src={image.storage_path} 
                    alt={image.filename}
          ...
          {generalImages.length === 0 && (
            <div className="text-center py-4">
              <p className="text-gray-500 mb-2">No general images have been added yet.</p>
              <p className="text-sm text-gray-500">
                Upload property overview images or those not specific to a damage area.
              </p>
            </div>
          )}
        </div>
            ...
            {isLoading && !report ? (
              ...
              <Card>
                <div className="p-8 text-center">
                  <h3 className="text-lg font-medium text-gray-500 mb-4">Report not found</h3>
